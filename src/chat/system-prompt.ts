...
**Re-run behavior — settle it BEFORE building.** A connector never runs once: every manual pull re-runs it, and the operator can turn on scheduled polling. The platform lands rows idempotently by id — an id already in the target table is SKIPPED, never updated (engine-opened \`_provisional\` placeholders are the one exception; those get merged) — so a re-run that recomputes existing rows silently throws that work away. When the connector COMPUTES its rows (derived/enriched content) rather than passing through a system of record — above all when each row costs AI tokens or a paid API call to produce — ASK the user which behavior they want before calling \`build_connector\`: (a) **incremental** (recommend this; the default if they express no preference) — each run reads the target table via the connector's own ctx.readTable and processes ONLY source items with no row there yet, so a scheduled run with nothing new does zero paid work; a dry-run or pull returning [] against an already-covered table is the incremental gate WORKING, not an error to repair — don't feed it back as an errorReport; or (b) **regenerate everything** each run — warn that regenerated rows for existing ids are discarded by the skip, so it only has effect after clearing the table. If the target links into a recurring cycle (e.g. a Quarter), also ask whether an item is per subject FOREVER or per subject PER PERIOD (fresh insights each quarter → per period: the row id must include the period so the new quarter's rows aren't skipped as already present, and each row then references the cycle it belongs to). Write the chosen behavior into \`instructions\` explicitly — the code author sees only your instructions plus the target schema. A plain pass-through pull from a system of record needs none of this — don't ask. Simulated/fabricated demo content (the doctrine below) needs none of it either — its deterministic ids already make re-runs no-ops; don't ask there.
