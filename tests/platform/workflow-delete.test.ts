// Workflow deletion cascade (the destructive lifecycle: "when a workflow is
// deleted, all its tables + entities + data are dropped"). Model-INDEPENDENT —
// builds its own org / workflow / ontology / data fixtures.
//
// Proves:
//   - the workflow's control-plane metadata is gone (workflow, ontology, versions,
//     branches, resource, resource markings, workflow-scoped role assignments)
//   - the workflow's data plane is gone (its gen__p<workflow>_ tables + EventLog)
//   - a SIBLING workflow in the same org is left completely untouched
//   - the system default workflow is protected (deletion refused)
...
import { newId, SYSTEM_ORG_ID, SYSTEM_WORKFLOW_ID } from "../../src/platform/ids.js";
import { assignRole, deleteWorkflow } from "../../src/platform/provisioning/index.js";
...
const orgId = newId();
const wsId = newId();
const projId = newId(); // the workflow under test (deleted)
const siblingId = newId(); // a second workflow that must survive
const aliceSub = `pd-alice-${SFX}`;
let aliceId: string;

const entity: EntitySchema = { name: `GenP${SFX}`, required: [], fields: [{ name: "id" }, { name: "label" }] };

function projCtx(workflowId: string): TenantContext {
  return { organizationId: orgId, principal: { id: aliceId, type: "identity" }, identityId: aliceId, subject: aliceSub, workflowId };
}

async function seedProject(workflowId: string, name: string) {
  ...
  // A gen__p<workflow>_ table with one row, created in THIS workflow's scope.
  await runWithTenant(projCtx(workflowId), async () => {
    await store.ensureTable(entity);
    await store.insert(entity.name, { id: newId(), label: `row-for-${name}` });
  });
  // An EventLog run-history row scoped to this workflow.
  await prisma.eventLog.create({
    data: { id: newId(), eventName: "X", eventRef: "#/x", boundedContext: "X", aggregateRoot: "X", aggregateId: newId(), role: "r", payload: "{}", organizationId: orgId, workflowId },
  });
  return { ontologyId };
}
...
beforeAll(async () => {
  ...
  // A workflow-scoped role grant on the doomed workflow (must be cascaded away).
  await assignRole({ organizationId: orgId, principalId: aliceId, principalType: "identity", roleKey: "editor", scopeType: "workflow", scopeId: projId });
  // A marking on the doomed workflow's ontology resource (must be cascaded away).
...
afterAll(async () => {
  // Best-effort teardown (the doomed workflow is already gone if the test passed).
...
const physTable = (workflowId: string) => `gen__p${workflowId.replace(/-/g, "")}_${entity.name}`;
...
describe("workflow deletion cascade", () => {
  it("refuses to delete the system default workflow", async () => {
    await expect(deleteWorkflow(SYSTEM_ORG_ID, SYSTEM_WORKFLOW_ID, aliceId)).rejects.toThrow(/system default workflow/i);
  });

  it("refuses a workflow that does not exist in the org", async () => {
    await expect(deleteWorkflow(orgId, newId(), aliceId)).rejects.toThrow(/not found/i);
  });

  it("drops the workflow's data plane, model metadata, grants, and history", async () => {
    // Preconditions: everything exists before the delete.
    expect(await tableExists(physTable(projId))).toBe(true);

    const result = await deleteWorkflow(orgId, projId, aliceId);
    expect(result.id).toBe(projId);
    expect(result.droppedModels).toBe(1);
    expect(result.droppedTables).toContain(physTable(projId));

    // Control plane gone.
    expect(await prisma.platWorkflow.findFirst({ where: { id: projId, organizationId: orgId } })).toBeNull();
    expect(await prisma.platOntology.count({ where: { organizationId: orgId, workflowId: projId } })).toBe(0);
    ...
    expect(await prisma.platResource.count({ where: { organizationId: orgId, workflowId: projId } })).toBe(0);
    expect(await prisma.platRoleAssignment.count({ where: { organizationId: orgId, scopeType: "workflow", scopeId: projId } })).toBe(0);

    // Data plane gone.
    expect(await tableExists(physTable(projId))).toBe(false);
    expect(await prisma.eventLog.count({ where: { organizationId: orgId, workflowId: projId } })).toBe(0);

    // The destructive act is audited.
    const audit = await prisma.platAuditEvent.findFirst({ where: { organizationId: orgId, action: "workflow.delete", targetRef: `workflow:${projId}` } });
    expect(audit).not.toBeNull();
    expect(audit?.decision).toBe("allow");
  });

  it("leaves the sibling workflow completely untouched", async () => {
    expect(await prisma.platWorkflow.findFirst({ where: { id: siblingId, organizationId: orgId } })).not.toBeNull();
    expect(await prisma.platOntology.count({ where: { organizationId: orgId, workflowId: siblingId } })).toBe(1);
    expect(await tableExists(physTable(siblingId))).toBe(true);
    expect(await prisma.eventLog.count({ where: { organizationId: orgId, workflowId: siblingId } })).toBe(1);
  });
});
