...
- **Agentic AI acting under guardrails moves governance onto the dashboard.** With EU AI Act high-risk traceability rules landing Aug 2026 and Gartner warning ~40% of agentic projects get cancelled over weak risk controls, the AI panel must be a *governance ledger*. **Status (built):** runtime/chat writes now carry an actor-origin tag (`actorKind`) and pass through the data-plane PDP guard (`guardData`, which audits every decision), so this category is computed live rather than pending instrumentation. → **AI vs Human Action Share**, **Override/Reversal Rate**, **Guardrail Block Rate**.
...
*How much is the AI doing, is it right, can I trust it to keep acting. This category is now BUILT — both prerequisites shipped: (i) actor-origin (`actorKind`) is stamped by `emit()`, (ii) the data-plane PDP (`guardData`) gates runtime/chat writes and audits each decision. The autonomy/override/guardrail-block-rate panel is computed in `src/twin/org-dashboard.ts`.*

| Indicator | Question | Computed | Viz | Pri | Data |
|---|---|---|---|---|---|
| **AI vs Human Action Share (autonomy mix)** | How much work is the AI carrying? | State-changing events tagged `ai` (origin = `/chat` write-tools) ÷ total, by type/step | Stacked bar per type | P1 | HAVE NOW (`actorKind` on `EventLog`, stamped by `emit()`; computed in `org-dashboard.ts`) |
| **Override / Reversal Rate** | Is the AI drifting out of its band? | `ai`-origin action negated/edited by a human on the same entity within window ÷ total AI actions | Trend + "top overridden" table | P1 | HAVE NOW (`actorKind` tag + same-aggregate ai→human correction heuristic) |
...
| **Guardrail / Policy Block Rate** | Are guardrails catching risky actions? | Guardrail-denial events per 100 attempted AI writes, by reason. **Built** — chat write-tools call the data-plane PDP (`guardData`), which audits each decision and records denied AI writes; the rate is computed cumulatively from the audit log | Small-multiples by reason | P2 | HAVE NOW (`guardData` on writes + audit log) |
...
| **Connector Freshness (SLA-coloured)** | Is any feed silently stale? | now − real `lastPullAt` per source vs per-source SLA; red/green | Traffic-light strip | P1 | PARTIAL — `ingestPull` now writes `lastPullAt` + `lastPullDurationMs` to the sidecar; still needs per-source SLA config + an org-level read path |
...
├──────────────────────────────────────────────────────────────────────────────────┤
│  AI ACTIVITY & TRUST    (live: actorKind tag + guardData PDP on writes shipped)     │
│  ┌─ Autonomy mix (P1) ──┐ ┌─ Override (P1) ─────┐ ┌─ Guardrail / approvals (P2) ───┐ │
│  │ AI ▓▓▓░ 31% / Human   │ │ Override   4.2%     │ │ PDP-on-write via guardData (live)│ │
│  │ actorKind tagged      │ │ actorKind tagged    │ │  + approval log (P2)             │ │
│  └─────────────────────┘ └─────────────────────┘ └─────────────────────────────────┘ │
├──────────────────────────────────────────────────────────────────────────────────┤
...
- **AI actor-origin tagging — DONE (shipped).** `EventLog` now carries actor identity: `emit()` (`src/events/bus.ts`) stamps `actorPrincipalId` (the human principal) and `actorKind` (`human|ai|system|adapter`, resolved by `currentActorKind()`) alongside `role`, `organizationId`, `workflowId`. The chat turn sets `ai` via a `withActorKind(...)` AsyncLocalStorage scope, so AI-driven chat writes are distinguishable from a human step-forward even though they share the `genericStep`/`genericNewInstance`/`ingestPull` path. With this in place, AI vs Human Share and Override Rate are computable — and computed in `src/twin/org-dashboard.ts`.
...
- **Guardrail — BUILT.** Chat write-tools now run through the data-plane PDP guard `guardData` (`src/platform/authz.ts`, invoked from `src/chat/tools.ts` and `src/http/adapter-routes.ts`); connector administer/edit and the other write actions are gated, the decision is audited with `actorKind` (`ai` when a chat write-tool triggered it), and a DENY raises. "Guardrail Block Rate" is therefore computed (cumulative denied-AI-writes ÷ attempted, from the audit log) in `src/twin/org-dashboard.ts`.
- **AI governance ledger:** the `guardData` write-guard already records each decision (allow/deny) to `PlatAuditEvent` (hash-chained, `src/platform/audit/index.ts`), so **Block Rate** and the actor-origin signals behind **Override Rate** are real today. Remaining work: the fuller proposal → accept/reject/revert ledger over chat write-tools that makes **Approval queue** and **Traceability coverage** real.
- **Connector freshness (SLA-coloured):** `ingestPull()` (`src/packs/ingest.ts`) now **writes `AdapterConfig.lastPullAt` + `lastPullDurationMs`** to the sidecar on every successful pull; still to do: persist `healthcheck()` results and an org→workflow→sidecar read path. Add per-source freshness SLA config so the strip can turn red. (`AdapterMode.at` is written today but is "mode last changed," **not** a per-pull freshness signal — do not conflate.)
...
3. **AI actor-origin + guardrail — RESOLVED (built).** Both prerequisites shipped: (a) `EventLog` now carries `actorPrincipalId`/`actorKind`, stamped by `emit()`; (b) chat write-tools run through the data-plane PDP guard `guardData`, which calls `recordAudit` on every decision (allow/deny). The autonomy / override / guardrail-block-rate panel is computed in `src/twin/org-dashboard.ts`. Remaining refinement: the richer AI proposal/approval ledger and full traceability coverage.
