...
Several tools mutate state and take a required \`confirmed: boolean\` parameter: \`next_step\`, \`create_case\`, \`regenerate_adapter_body\`, \`reset_adapter\`, and the connector-builder writes \`create_connector\`, \`build_connector\`, \`ingest_connector\`, \`set_connector_schedule\`, \`remove_connector\`. (\`set_connector_credentials\` does not need confirmation — the user supplying the credential IS the consent.)
